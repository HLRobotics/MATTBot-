...
class ChatMain(ttk.Frame):
    ...
    def __init__(self, config_file_path="socketchat.xml"):
        ...
        self.master.title("MATTBOT LAN CHAT")
    ...
    def load_config(self, config_file_path=None):
        ...
        if not config_file_path:
            ...
            config_file_path = tkfiledialog.askopenfilename(parent=self, filetypes=_filetypes, title="Load Configuration", defaultextension=".xml", initialdir=os.path.abspath("."))
    ...
    def populate_connection(self, conn_elem):
        ...
        self.tree.insert("", "end", iid=conn_elem.get("id"), text=conn_elem.findtext("displayname"), tags="#entry")
...
class ChatWindow(tk.Toplevel):
    ...
    def __init__(self, iid, displayname, address, port):
        ...
        self.display = scrolledtext.ScrolledText(background_frame, height=18, borderwidth=2, relief=tk.SUNKEN, state=tk.DISABLED, wrap=tk.WORD)
        self.input = scrolledtext.ScrolledText(background_frame, height=10, wrap=tk.WORD, borderwidth=2, relief=tk.SUNKEN)
        send_button = ttk.Button(background_frame, text="Send", width=15, command=self.send_and_display_msg)
        menubar = tk.Menu(self)
        filemenu = tk.Menu(menubar, tearoff=0)
        # configure GUI widgets
        ...
        filemenu.add_command(label="Add Connection", command=lambda: NewConnection(self.master.children["chatmain"].config, host=self.address))
        filemenu.add_separator()
...
class NewConnection(tk.Toplevel):
    ...
    def _add(self, *args, **kwargs):
        ...
        if conn_elem is None:
            new_conn_elem = create_elem_with_subs(self.config.getroot(), "connection", {"id": str(uuid4())}, {"hostname": hostname, "displayname": self.displayname.get(), "address": addr})
...
def init_config():
    ...
    create_elem_with_subs(tree.getroot(), "request_server", grandchild_elem_dict={"address": "", "port": "12141"})
    return tree
